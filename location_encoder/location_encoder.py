...
class LocationEncoder(nn.Module):
    ...
    def _load_weights(self):
        ...
        checkpoint = torch.load(f"./location_encoder_weights.pth", map_location='cpu')
        ...
        for name, param in self.named_parameters():
            if name not in k:
                ...
                incompatible_shape[name] = (param.shape, checkpoint[name].shape)
            else:
                pretrained_encoder[name] = checkpoint[name]
        self.load_state_dict(pretrained_encoder, strict=False)
        # self.parameters_warning(missing, incompatible_shape, logger)
        # print("missing keys: ", missing)
        # print("incompatible shape: ", incompatible_shape)
        # print(f"Loaded encoder weights from {self.encoder_weights}")
        # self.load_state_dict(torch.load(f"/home/zl22853/code/SatMAE_new/location_encoder/location_encoder/model/weights/location_encoder_weights.pth"),strict=False)

    def forward(self, location):
        location = equal_earth_projection(location)
        location_features = torch.zeros(location.shape[0], 768).to(location.device)
